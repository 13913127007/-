...
layui.define(['config', 'crab', 'layer', 'laytpl', 'element', 'form'], function (exports) {
    ...
    var index = {
            ...
            if (id) {
                ...
                $.each(menus, function (i, m) {
                    ...
                    html += '<li class="layui-nav-item' + cls + '" lay-unselect>' +
                        '<a ' + aClass + ' data-id="' + m.id + '" href="' + uri + '" '+(m.uri?"target=\"_blank\"":"")+'\n>' + m.name + '</a>' +
                        '</li>';
                });
                $('#topMenu').append(html);
            }
        ...
        initRouter: function (menus) {
            Q.reg("business-building-building-form", function () {
                index.loadView("business-building-building-form", '/components/business/building/building-form.html','房间表单');
            });
        ...
        bindEvent: function () {
            ...
            $('#myInfo').click(function () {
                crab.popupCenter({
                    title: '个人信息',
                    path: '/components/sys/user-info-form.html'
                });
            });
            // 退出登录
